...
## Datasets (see &sect;6 in the paper and Appx E)
...
## Algorithms (see &sect;6 in the paper)
...
## Reproducing results (see &sect;6 in the paper)
...
### Generating plots (see &sect;6 in the paper)
1. For the effect of $k$ ([n_clusters_plot.py](n_clusters_plot.py)):
   ...
   2. Fair Centroid Objective vs Fair $k$-means:
...
3. For the disparity trends ([faircentroid_plot.py](faircentroid_plot.py)):
   ...
   ```bash
   python faircentroid_plot.py --dataset_name Adult_race --n_clusters 3 --init_method kmeans_plusplus --random_state 60 --lambda_ 0.5
   ```
   The following plot was generated by our results (see Figure 10 in the paper):
   [Adult_race.k3.r60.pdf](data/expts/Adult_race/analysis/Adult_race.k3.r60.pdf).
