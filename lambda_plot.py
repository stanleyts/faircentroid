...
if __name__ == '__main__':
    ...
    aggregated_scores = pd.read_csv(aggregated_scores_path, index_col=[0, 1, 2, 3, 4])
    aggregated_scores = aggregated_scores.xs(args.INIT_METHOD, axis='index', level='init_method')
    if len(aggregated_scores.index.unique('n_clusters')) != 1:
        raise ValueError
    n_clusters, = aggregated_scores.index.unique('n_clusters')
    aggregated_scores = aggregated_scores.droplevel('n_clusters', axis='index')
    ...
    for metric in aggregated_scores.index.unique('metric'):
        lloyd_score = aggregated_scores.loc[(metric, 'Lloyd', 0), 'mean']
        data = aggregated_scores.xs((metric, 'Fair-Centroid'), axis='index', level=('metric', 'algorithm'))['mean']
        data.name = 'Fair-Centroid'
        _ = plt.figure()
        ylabel = metric_dict.get(metric, metric) if args.DATASET_NAME == 'Adult_sex' else None
        ax = data.plot.line(style='.-', color='tab:green', xlabel=r'$\lambda$', ylabel=ylabel)
        _ = ax.axhline(y=lloyd_score, color='tab:blue', label='Lloyd', linestyle='--')
        _ = ax.set_xlim(left=0)
        _ = ax.set_ylim(bottom=0)
        ax.xaxis.set_major_locator(ticker.MultipleLocator(.2))
        ax.legend(reverse=True, title=None)
        if args.DATASET_NAME != 'Adult_sex':
            ax.legend().set_visible(False)
        filename = f"lambda_ vs {metric} - {args.DATASET_NAME}.pdf"
